...
class AnthropicChatModel(InferenceAPIModel):
    def __init__(
        ...
        _base_url = os.environ.get("ANTHROPIC_BASE_URL", None)
        if anthropic_api_key:
            self.aclient = AsyncAnthropic(api_key=anthropic_api_key, **({"base_url": _base_url} if _base_url else {}))
        else:
            self.aclient = AsyncAnthropic(**({"base_url": _base_url} if _base_url else {}))
...
class AnthropicModelBatch:
    def __init__(self, anthropic_api_key: str | None = None):
        _base_url = os.environ.get("ANTHROPIC_BASE_URL", None)
        if anthropic_api_key:
            self.client = anthropic.Anthropic(api_key=anthropic_api_key, **({"base_url": _base_url} if _base_url else {}))
        else:
            self.client = anthropic.Anthropic(**({"base_url": _base_url} if _base_url else {}))
